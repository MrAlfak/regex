...
> **Una lista de verificación integral y práctica** de las tareas más importantes que deben realizarse para mantener la seguridad durante el diseño, desarrollo, pruebas e implementación de API.
...
- 🇪🇸 [Español (Spanish)](./README.es.md) - Actual
...
Este proyecto es una **lista de verificación integral y práctica** para la seguridad de API que ayuda a los desarrolladores a diseñar, desarrollar e implementar sus APIs de forma segura.

### ✨ Características

- 🔒 **Cobertura Integral**: Incluye todos los aspectos de seguridad de API desde autenticación hasta CI/CD
- 📝 **Práctico y Utilizable**: Cada elemento incluye explicaciones y ejemplos prácticos
- 🎯 **Bien Organizado**: Categorizado por dominios de seguridad
- 🔄 **Actualizado Continuamente**: Se mantiene al día con los últimos estándares y mejores prácticas
- 🌍 **Soporte Multiidioma**: Disponible en persa, inglés, árabe y español

### 🎯 ¿Para quién es?

- Desarrolladores Backend que construyen APIs
- Equipos de DevOps y Seguridad
- Arquitectos de software y Líderes Técnicos
- Estudiantes y entusiastas de la seguridad

### 🚀 Inicio Rápido

1. Da una estrella a este repositorio para mantenerte actualizado
2. Revisa la lista de verificación antes de cada implementación
3. Marca los elementos relevantes para tu proyecto
4. Envía un Pull Request si es necesario
...
Si este proyecto te ha sido útil, puedes apoyarnos a través de los siguientes métodos:
...
- [Cómo Usar](#-cómo-usar)
- [Lista de Verificación Rápida](#-lista-de-verificación-rápida)
- [🔐 Autenticación y Autorización](#-autenticación-y-autorización)
- [🌐 Acceso y Red](#-acceso-y-red)
- [📥 Entrada y Validación](#-entrada-y-validación)
- [⚙️ Procesamiento y Lógica de Negocio](#️-procesamiento-y-lógica-de-negocio)
- [📤 Salida y Respuestas](#-salida-y-respuestas)
- [💾 Almacenamiento y Datos](#-almacenamiento-y-datos)
- [📊 Monitoreo, Observabilidad y Respuesta](#-monitoreo-observabilidad-y-respuesta)
- [🔧 Manejo de Errores y Versionado](#-manejo-de-errores-y-versionado)
- [🔗 CORS y Seguridad del Navegador](#-cors-y-seguridad-del-navegador)
- [🔑 Gestión de Claves y Secretos](#-gestión-de-claves-y-secretos)
- [🌍 Webhooks y Seguridad de Eventos](#-webhooks-y-seguridad-de-eventos)
- [📚 Documentación y Documentación API](#-documentación-y-documentación-api)
...
Esta lista de verificación actúa como un **tablero Kanban de Seguridad**:

- ✅ Antes de cada lanzamiento, revisa cada categoría y marca los elementos para aclarar el estado de seguridad
- 📝 Si un elemento no se aplica a tu proyecto, documenta la razón del incumplimiento para que sea defendible en futuras auditorías
- 🎯 Mueve los elementos que requieren acción al backlog y asigna propietarios/plazos
- 🔄 Revisa y actualiza este documento periódicamente (por ejemplo, trimestralmente)

---

## ⚡ Lista de Verificación Rápida

Estos elementos **deben** ser revisados antes de cada implementación en producción:

- [ ] 🔴 **Los entornos están separados**: Dev/Test y Prod están completamente separados (entorno, datos, cuentas)
- [ ] 🔴 **Los secretos están seguros**: Los secretos no se almacenan en código, correo electrónico o aplicaciones de mensajería, y tienen rotación periódica
- [ ] 🔴 **Rate Limiting está activo**: Ningún endpoint es público sin Rate Limit
- [ ] 🔴 **El monitoreo está activo**: Los errores y eventos de seguridad se monitorean con alertas accionables
- [ ] 🔴 **HTTPS es obligatorio**: Todo el tráfico pasa por HTTPS y HTTP redirige
- [ ] 🔴 **La autenticación está protegida**: Todos los endpoints sensibles están protegidos con autenticación
...
- [ ] No uses `Basic Auth`. En su lugar, usa métodos de autenticación estándar ([JWT](https://jwt.io/), [OAuth 2.0](https://oauth.net/2/), o [OpenID Connect](https://openid.net/connect/))
- [ ] No reinventes la rueda para `autenticación`, `generación de tokens` y `almacenamiento de contraseñas`. Usa bibliotecas estándar y probadas
- [ ] Implementa límites de `intentos máximos de reintento` para inicio de sesión (por ejemplo, 5 intentos en 15 minutos) y límites de frecuencia de inicio de sesión
- [ ] Cifra todos los datos sensibles (en tránsito y en reposo)
- [ ] Usa Autenticación Multi-Factor (MFA/2FA) para cuentas sensibles
- [ ] Define un tiempo de espera de sesión apropiado (por ejemplo, 30 minutos de inactividad)
- [ ] Invalida todas las sesiones anteriores después del cambio de contraseña
- [ ] Usa una política de contraseñas fuerte (mínimo 12 caracteres, combinación de mayúsculas/minúsculas, números, caracteres especiales)
...
Puedes hacer fork de este repositorio fácilmente, aplicar los cambios que desees y enviar un pull request.

Si tienes alguna pregunta, envíanos un correo electrónico a `info@zahedalfak.ir`.
...
[API-Security-Checklist en GitHub](https://github.com/MrAlfak/API-Security-Checklist)

</div>

---

## 📋 Lista TODO de Traducción

Esta sección rastrea el progreso de la traducción de la Lista de Verificación de Seguridad API a diferentes idiomas.

### ✅ Traducciones Completadas
- [x] 🇮🇷 Persa (فارسی) - `Readme.md`
- [x] 🇬🇧 Inglés - `README.en.md`
- [x] 🇸🇦 Árabe (العربية) - `README.ar.md`
- [x] 🇪🇸 Español - `README.es.md`
...
¡Aceptamos contribuciones! Si deseas ayudar a traducir esta lista de verificación a tu idioma:

1. Haz fork de este repositorio
2. Crea un nuevo archivo `README.[código-idioma].md` (por ejemplo, `README.es.md` para español)
3. Traduce el contenido de `README.en.md`
4. Actualiza la lista de idiomas en todos los archivos README
5. Envía un Pull Request

**Nota**: Por favor mantén la misma estructura, formato y uso de emojis que la versión en inglés.

---

**Última Actualización**: Consulta el historial de commits para las últimas actualizaciones de traducción.
